...
class SocketServer extends EventEmitter {
	...
	onError(err) {
		if (err.code === 'EADDRINUSE') console.log('Warning: Cannot start `' + this.name + '` service on port ' + this.port + '. Error Code: EADDRINUSE, Address already in use.');
		else if (err.code === 'EACCES') console.log('Warning: Cannot start `' + this.name + '` service on port ' + this.port + '. Error Code: EACCES, Permission Denied.');
...
class FtpSocketServer extends SocketServer {
	start() {
		new FtpSrv('ftp://' + config.server_ip + ':' + this.port, {
